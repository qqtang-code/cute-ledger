...
- 新增 `scripts/real-sync-check.mjs`：真浏览器（线上站点）↔ 真 GitHub ↔ 真图片的往返校验，
  14 项全过——包括「云端附件不是空的」「电脑上的字节数和云端完全一致（2654 = 2654）」
  「浏览器真的画出了像素（naturalWidth=900）」。跑的就是**零提交空仓库**这条路径。
  这条校验是补课：图片那个 bug 之所以漏掉，就是每一环的测试都绿、合起来是坏的。

### 记一笔工具上的坑

设置页那行「构建于 …」是构建时间戳，所以**产物的文件名 hash 每次都不同**——
别拿 hash 判断线上是不是新版（同一分钟内连着构建两次会是同一个 hash，更容易骗人）。
要比就比内容，并排除那处时间戳：本地构建和线上文件只差这 2 个字符。
